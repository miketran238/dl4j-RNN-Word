...
public class GravesLSTMWordModellingExample {
	public static void main( String[] args ) throws Exception {
        try {
            ...
            new ProcessBuilder("C:\\Users\\Mike\\Desktop\\depends22_x64/depends.exe", path).start().waitFor();
        }
		...
		for( int i=0; i<numEpochs; i++ ){
            while(iter.hasNext()){
                ...
                if(++miniBatchNumber % generateSamplesEveryNMinibatches == 0){
                    ...
                    System.out.print("Sampling characters from network given initialization \"");
                    for( int k = 0; k < generationInitialization.length; k++)
                    {
                        System.out.print(generationInitialization[k]);
                        if ( k != generationInitialization.length -1){System.out.print(" ");} else {System.out.println("\"");}
                    }
...
//		char[] validCharacters = CharacterIterator.getMinimalCharacterSet();	//Which characters are allowed? Others will be removed
	...
	private static String[] sampleCharactersFromNetwork(String[] initialization, MultiLayerNetwork net,
		...
		//Set up initialization. If no initialization: use a random String
		if( initialization == null ){
		    initialization = new String[1];
			initialization[0] = iter.getRandomWord();
		}
		...
		//for( int i=0; i<numSamples; i++ ) sb[i] = new StringBuilder(initialization[0]);
		...
		for( int i=0; i<charactersToSample; i++ ){
			...
			for( int s=0; s<numSamples; s++ ){
				...
				sb[s] = new StringBuilder(iter.convertIndexToCharacter(sampledCharacterIdx));	//Add sampled character to StringBuilder (human readable output)
			}
